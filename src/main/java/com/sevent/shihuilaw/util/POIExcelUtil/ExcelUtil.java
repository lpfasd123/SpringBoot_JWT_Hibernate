package com.sevent.shihuilaw.util.POIExcelUtil;
...
//        sheet.setColumnWidth(0, 256*100);
        ...
        write2Sheet(sheet, headers, dataset, pattern, cellStyle);
    ...
    public static <T> void exportExcel(List<ExcelSheet<T>> sheets, OutputStream out, String pattern) {
        ...
        for (ExcelSheet<T> sheet : sheets) {
            ...
            write2Sheet(hssfSheet, sheet.getHeaders(), sheet.getDataset(), pattern, cellStyle);
        }
        try {
            workbook.write(out);
        } catch (IOException e) {
    ...
    private static <T> void write2Sheet(HSSFSheet sheet, Map<String,String> headers, Collection<T> dataset,
                                        String pattern, CellStyle cellStyle) {
        //时间格式默认"yyyy-MM-dd"
            ...
            try {
                if (t instanceof Map) {
                    ...
                    while (it2.hasNext()){
                        ...
                        cell.setCellStyle(cellStyle);
                        sheet.setColumnWidth(0,value.toString().length());
